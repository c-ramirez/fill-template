...
public class TemplateFiller {
    ...
    static final String[] regexChars = new String[]{"\\", ".", "+", "*", "?", "^", "$", "(", ")", "[", "]", "{", "}", "|"};
    public static final String BLANK = "";
    public static final String BACKSLASH = "\\";

    public static String getTemplateWithValues(String template, TemplateData data) {
        return setFieldsValueInTemplate(setParametersValuesInTemplate(template, data.getParameters()),
                data.getRows());
    }
            ...
            for (TemplateRow tempRow : templateRow) {
                final Pattern fieldPattern = Pattern.compile(
                        String.format(FIELD_REGEX,
                                tempRow.getFields().stream().map(Pair::getKey).collect(Collectors.joining("|"))),
                        Pattern.DOTALL);
    ...
    private static String escapeSpecialCharacter(String word) {
        ...
        for (String regexChar : regexChars)
            word = word.replace(regexChar, BACKSLASH + regexChar);
        return word;
    }
}
